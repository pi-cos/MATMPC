...
Definition of GP with covariance defined  by stationary kernels.
"""
...
class RBF(Stationary_GP):
    ...
    def get_covariance(self, X1, X2=None, flg_noise=False):
        ...
        if flg_noise & self.GP_with_noise:
            N = X1.size()[0]
            return torch.exp(self.scale_log)*torch.exp(-self.get_weigted_distances(X1, X2)) + self.get_sigma_n_2()*torch.eye(N, dtype=self.dtype, device=self.device)
        else:
            return torch.exp(self.scale_log)*torch.exp(-self.get_weigted_distances(X1, X2))
