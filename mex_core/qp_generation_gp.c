...
mexFunction(int nlhs,mxArray *plhs[],int nrhs,const mxArray *prhs[])
{
    ...
    // GP for sim_erk_gp
    bool gp_flag = mxGetScalar( mxGetField(prhs[2], 0, "gp_flag") );
    double *xn_gp  = mxGetPr( mxGetField(prhs[2], 0, "x_gp") );
    double *xn_ode = mxGetPr( mxGetField(prhs[2], 0, "x_ode") );
    double *A_gp = mxGetPr( mxGetField(prhs[2], 0, "A_gp") );
    double *B_gp = mxGetPr( mxGetField(prhs[2], 0, "B_gp") );
    double *A_ode = mxGetPr( mxGetField(prhs[2], 0, "A_ode") );
    double *B_ode = mxGetPr( mxGetField(prhs[2], 0, "B_ode") );
    ...
    for(i=0;i<N;i++){
        ...
        switch(sim_method){
            ...
            case 4:
                ...
                out->Su = B + i*nx*nu;
                // GP
                out->xn_gp  = xn_gp  + i*nx;
                out->xn_ode = xn_ode + i*nx;
                out->A_gp = A_gp + i*nx*nx;
                out->B_gp = B_gp + i*nx*nu;
                out->A_ode = A_ode + i*nx*nx;
                out->B_ode = B_ode + i*nx*nu;
