...
void mex_adj_GP_fun(int resc, mxArray *resv[], int argc, const mxArray *argv[]) {
  ...
  if (--argc>=0) arg[1] = casadi_from_mex(argv[1], w+4, casadi_s1, w+169);
  if (--argc>=0) arg[2] = casadi_from_mex(argv[2], w+5, casadi_s2, w+169);
  if (--argc>=0) arg[3] = casadi_from_mex(argv[3], w+160, casadi_s0, w+169);
  if (--argc>=0) arg[4] = casadi_from_mex(argv[4], w+164, casadi_s3, w+169);
  casadi_real* res[1] = {0};
  --resc;
  res[0] = w+164;
  i = adj_GP_fun(arg, res, iw, w+169, 0);
