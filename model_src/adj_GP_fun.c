...
#else
  #define CASADI_PREFIX(ID) adj_GP_fun_ ## ID
#endif
...
/* adj_GP_fun:(i0[4],i1,i2[1415],i3[4],i4[0])->(o0[5]) */
...
#ifdef MATLAB_MEX_FILE
void mex_adj_GP_fun(int resc, mxArray *resv[], int argc, const mxArray *argv[]) {
  int i, j;
  if (argc>5) mexErrMsgIdAndTxt("Casadi:RuntimeError","Evaluation of \"adj_GP_fun\" failed. Too many input arguments (%d, max 5)", argc);
  if (resc>1) mexErrMsgIdAndTxt("Casadi:RuntimeError","Evaluation of \"adj_GP_fun\" failed. Too many output arguments (%d, max 1)", resc);
  int *iw = 0;
  casadi_real w[2844];
  const casadi_real* arg[5] = {0};
  if (--argc>=0) arg[0] = casadi_from_mex(argv[0], w, casadi_s0, w+1429);
  if (--argc>=0) arg[1] = casadi_from_mex(argv[1], w+4, casadi_s1, w+1429);
  if (--argc>=0) arg[2] = casadi_from_mex(argv[2], w+5, casadi_s2, w+1429);
  if (--argc>=0) arg[3] = casadi_from_mex(argv[3], w+1420, casadi_s0, w+1429);
  if (--argc>=0) arg[4] = casadi_from_mex(argv[4], w+1424, casadi_s3, w+1429);
  casadi_real* res[1] = {0};
  --resc;
  res[0] = w+1424;
  i = adj_GP_fun(arg, res, iw, w+1429, 0);
  if (i) mexErrMsgIdAndTxt("Casadi:RuntimeError","Evaluation of \"adj_GP_fun\" failed.");
...
void mexFunction(int resc, mxArray *resv[], int argc, const mxArray *argv[]) {
  ...
  if (!buf_ok) {
    /* name error */
  } else if (strcmp(buf, "adj_GP_fun")==0) {
    return mex_adj_GP_fun(resc, resv, argc, argv);
  }
  mexErrMsgTxt("First input should be a command string. Possible values: 'adj_GP_fun'");
}
#endif
#ifdef __cplusplus
} /* extern "C" */
