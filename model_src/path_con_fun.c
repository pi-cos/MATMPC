...
void mex_path_con_fun(int resc, mxArray *resv[], int argc, const mxArray *argv[]) {
  ...
  if (--argc>=0) arg[0] = casadi_from_mex(argv[0], w, casadi_s0, w+160);
  if (--argc>=0) arg[1] = casadi_from_mex(argv[1], w+4, casadi_s1, w+160);
  if (--argc>=0) arg[2] = casadi_from_mex(argv[2], w+5, casadi_s2, w+160);
  casadi_real* res[1] = {0};
  --resc;
  res[0] = w+160;
  i = path_con_fun(arg, res, iw, w+160, 0);
