...
void mex_gp_jac_fun(int resc, mxArray *resv[], int argc, const mxArray *argv[]) {
  ...
  if (--argc>=0) arg[0] = casadi_from_mex(argv[0], w, casadi_s0, w+180);
  if (--argc>=0) arg[1] = casadi_from_mex(argv[1], w+4, casadi_s1, w+180);
  if (--argc>=0) arg[2] = casadi_from_mex(argv[2], w+5, casadi_s2, w+180);
  if (--argc>=0) arg[3] = casadi_from_mex(argv[3], w+160, casadi_s3, w+180);
  casadi_real* res[2] = {0};
  --resc;
  res[0] = w+160;
  if (--resc>=0) res[1] = w+176;
  i = gp_jac_fun(arg, res, iw, w+180, 0);
